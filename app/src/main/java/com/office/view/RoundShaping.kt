 ...
 */
class RoundShaping @JvmOverloads constructor(
    context: Context,
    ...
    init {
        ...
        isCircle = ta.getBoolean(R.styleable.RoundShaping_is_circle, false)
        //一定要加上这句代码，否则有可能绘制不出来
        setLayerType(LAYER_TYPE_SOFTWARE, null)
        paint.color = ta.getColor(R.styleable.RoundShaping_clip_color, Color.WHITE)
    ...
    override fun onDraw(canvas: Canvas?) {
        ...
        canvas?.drawRoundRect(
            ...
            if (isCircle) width / 2f else corner_radius,
            if (isCircle) width / 2f else corner_radius, paint
        )
        paint.xfermode = null
