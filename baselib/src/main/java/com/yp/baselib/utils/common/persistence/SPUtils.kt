package com.kotlinlib.common.persistence
...
interface SPUtils {
    ...
    companion object {
        ...
        fun <T> getSP(ctx: Context, key: String, defaultObject: T): T {
            val sp = ctx.getSharedPreferences(FILE_NAME, Context.MODE_PRIVATE)
            return when (defaultObject) {
                is String -> sp.getString(key, defaultObject.toString()) as T
                is Int -> sp.getInt(key, defaultObject.toInt()) as T
                is Boolean -> sp.getBoolean(key, defaultObject) as T
                is Float -> sp.getFloat(key, defaultObject.toFloat()) as T
                is Long -> sp.getLong(key, defaultObject.toLong()) as T
                else -> Any() as T
            }
        }
    ...
    fun Context.getSP(key: String, defaultObject: Any): Any {
        ...
        return when (defaultObject) {
            is String -> sp.getString(key, defaultObject.toString()) as Any
